    ...
    function drewlabs_core_get_attribute($obj, $key, $default = null)
    {
        if (is_object($obj)) {
            ...
            if ($refObject->hasProperty($key)) {
                ...
                $publicProperties = array_map(static function (ReflectionProperty $prop) {
                    return $prop->getName();
...
if (!function_exists('drewlabs_core_create_attribute_getter')) {
    ...
    function drewlabs_core_create_attribute_getter($key, $default = null)
    {
        return static function ($obj) use ($key, $default) {
            return drewlabs_core_validate_attribute_name(static function () use ($obj, $key, $default) {
        ...
        return static function ($obj) use ($key, $value) {
            ...
            if (is_array($key)) {
                return array_reduce($key, static function ($carry, $current) {
                    return drewlabs_core_validate_attribute_name(static function () use ($carry, $current) {
                        return drewlabs_core_recursive_set_attribute($carry, ...$current);
                    })(...$current);
                }, $obj);
            }
            // return \drewlabs_core_recursive_set_attribute($obj, $key, $value);
            return drewlabs_core_validate_attribute_name(static function () use ($obj, $key, $value) {
                return drewlabs_core_recursive_set_attribute($obj, $key, $value);
            })($key, $value);
        };
    }
